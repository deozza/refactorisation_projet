...
class GameController extends AbstractController
{
    ...
    public function invitePlayerToGame(Request $request, EntityManagerInterface $entityManager, $id, $playerRightId): JsonResponse
    {
        $currentUserId = $request->headers->get('X-User-Id');

        if (empty($currentUserId) || !ctype_digit($id) || !ctype_digit($playerRightId) || !ctype_digit($currentUserId)) {
            return new JsonResponse('User or game not found', 401);
        }
        ...
        if ($game === null || $game->getState() === 'ongoing' || $game->getState() === 'finished') {
            return new JsonResponse('Game not found or already started', 404);
        }

        $playerRight = $entityManager->getRepository(User::class)->find($playerRightId);

        if ($playerRight === null || $playerLeft->getId() === $playerRight->getId()) {
            return new JsonResponse('Invalid player', 404);
        }
        ...
        return $this->json($game, ['Content-Type' => 'application/json;charset=UTF-8']);
    }
